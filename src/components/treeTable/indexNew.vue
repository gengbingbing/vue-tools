					...
					<td 
						...
						<div class="arrow filter">
							<el-popover
								...
								trigger="click">
								<div class="textAlign">
									<el-input v-model="filterInputVal" size="mini" suffix-icon="el-icon-search" placeholder="请输入内容" style="margin-bottom: 10px"></el-input>
									<el-button size="mini" type="primary" @click="filterBtnClick(item.id)">确定</el-button>
									<el-button size="mini" @click="cancleBtnClick(item.id)">取消</el-button>
								</div>
								<i slot="reference" class="el-icon-arrow-down"></i>
							</el-popover>
						</div>
						<div class="arrow sort">
							<i class="el-icon-sort" @click="isSort(item.id)"></i>
						</div>
					</td>
				</tr>
			</table>
...
<script>
import treeItem from '@/components/treeTable/tree-item'
	export default {
		name: 'treeTable',
		props: ['list', 'op'],
		data() {
		...
		methods: {
			...
			handleSizeChange(val) {
				...
				this.$emit('handleSizeChange', handleSizeChange)
			},
			...
			isSort(col) {
				this.$emit('orderByFunc', col)
			},
			filterBtnClick(col) {
				let obj = {
					key: col,
					value: this.filterInputVal
				}
